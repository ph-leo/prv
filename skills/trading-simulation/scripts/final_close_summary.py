...
def get_index_hist(symbol="000001"):
    """获取指数历史数据"""
    print(f"正在获取指数 {symbol} 历史数据...")
    try:
        df = ak.index_zh_a_hist(symbol=symbol)
        if len(df) > 0:
            latest = df.iloc[-1]
            print(f"最新数据: {latest['日期']} 收盘价 {latest['收盘']:.2f}")
            return df
    except Exception as e:
        print(f"获取指数历史失败: {e}")
    return None
...
def generate_report():
    ...
    # 尝试获取行情数据
    for func in [get_stock_spot_old]:
        stock_df = retry_request(func)
...
def main():
    ...
    # 检查是否在交易时间（15:00-16:00）
    now = datetime.now()
    hour = now.hour
    minute = now.minute
    
    print(f"当前时间: {now.strftime('%Y-%m-%d %H:%M:%S')}")
    if hour < 15 or (hour == 15 and minute < 5):
        print("⚠️ 现在距离A股收盘时间（15:00）还有至少5分钟")
        print("建议在15:05后再运行此脚本")
        print("")
...
if __name__ == "__main__":
    ...
    if not has_data:
        print("\n⚠️ 警告：未能成功获取A股行情数据")
        print("可能的原因：")
        print("1. 数据源（东方财富）暂时不可访问")
        print("2. 网络连接问题")
        print("3. akshare库的API接口变更")
        print("\n建议：")
        print("1. 稍后重试")
        print("2. 检查网络连接")
        print("3. 更新 akshare: pip install --upgrade akshare")
