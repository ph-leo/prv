...
# ==================== 胸部基础 Prompt V7.4 ====================
CHEST_PROMPT_V74 = """你是一名资深放射科医生，请对该胸部X光片进行系统诊断分析。
...
### 2. 肺部评估 (Lungs)
**肺野透亮度**: 双侧对称，有无局部透亮度增高/降低
**肺纹理**: 走行自然，分支正常，有无增粗、紊乱、减少
**肺实质**: 寻找异常密度影
  - 浸润影：密度增高，边界模糊（肺炎）
  - 结节影：类圆形致密影，边界可清可不清
  - 空洞**: 壁厚薄，液平面，内容物
...
### 4. 纵隔评估 (Mediastinum)
**心影**: 大小、形态、位置，心缘obliteration
**肺门**: 形态、密度，有无增大
**纵隔轮廓**: 清晰，有无移位、隆起

### 5. 血管钙化 (Vascular Calcification) - 重要！
**主动脉**: 主动脉弓、降主动脉壁钙化
**冠状动脉**: 心影内钙化灶
**血管钙化**: 其他血管钙化
...
1. **系统性**：按上述流程逐一评估
2. **精确性**：病变部位精确到肺叶/段、骨骼具体部位
3. **描述性**：大小、形态、边界、密度、周围反应
4. **鉴别性**：给出可能诊断及依据
5. **置信度**：每个诊断标注置信度（高/中/低）
...
LESION_MAPPING_V74 = {
    ...
    '骨质增生': ['hyperplasia', 'osteophyte', 'spur', 'degenerative'],
...
def main():
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f'- 胸部专用: {stats["chest"]["total"]} 例\n\n')

        f.write('## 各病变类型识别率\n\n')
        f.write('| 病变类型 | 预期病例数 | 检出 | 识别率 |\n')
        f.write('|---------|---------|--------|--------|\n')
        for lesion, stats_item in sorted(lesion_stats.items(), key=lambda x: x[1]['rate'], reverse=True):
            f.write(f"| {lesion} | {stats_item['expected']} | {stats_item['detected']} | **{stats_item['rate']:.1f}%** |\n")
        f.write('\n')
